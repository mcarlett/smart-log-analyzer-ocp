...
| `components.<name>.expose` | `false` | Create an edge TLS Route (Services are always created for enabled components) |
...
4. The application container sources these files before starting: `. /vault/secrets/env && exec /opt/jboss/container/java/run/run-java.sh`
...
| HTTP server | `camel-quarkus-platform-http` | `camel.rest.component=platform-http` (Spring Boot auto-configuration) |
...
| Observability metrics | Port 9876 (`/observe/metrics`) | Port 9876 (`/observe/metrics`) |
...
The pipelines and Helm chart create several resources at runtime. The following tables summarize what is pre-applied from files, what is generated by the pipeline tasks, and what the Helm chart creates.

### Pre-applied (from `resources/` and `triggers/`)
...
| `poll-source-repo-config` | ConfigMap | `triggers/poll-source-repo-config.yaml` | Polling CronJob configuration (source repo, runtime, deps) |
...
|---|---|---|
| `source-repo-state` | ConfigMap | Stores the last known commit SHA for change detection |
...
| `<app-name>` | Service | Service for all enabled components (HTTP + management ports) |
...
| OpenAI secrets in Vault | Vault KV | OpenAI/LLM credentials stored at `secret/openai` (see [Vault Agent Injector](#vault-agent-injector-helm-chart)) |
